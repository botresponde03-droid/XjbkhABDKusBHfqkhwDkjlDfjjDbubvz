...
AUTHORIZED_ROLE_ID=1513775165283238049
...
# ================= COMANDO !editar rD =================
@bot.command()
async def editar(ctx,subcommand=None):
    if subcommand!="rD":
        return
    ...
    # Pedir mensaje
    await ctx.send("📝 Envía el mensaje que quieres guardar:")
    try:
    ...
    # Pedir imagen
    await ctx.send("🖼️ Envía la imagen (o URL de imagen):")
    try:
        img_msg=await bot.wait_for("message",check=lambda m:m.author==ctx.author,timeout=60)

        # Si tiene adjuntos, usar la URL del adjunto
        if img_msg.attachments:
            imagen_url=img_msg.attachments[0].url
        else:
            imagen_url=img_msg.content
    except asyncio.TimeoutError:
        await ctx.send("⏱️ Tiempo agotado.")
        return
    ...
    await ctx.send("✅ Configuración guardada correctamente.")
...
async def config(ctx):
    ...
    if not config_data or "rD_config" not in config_data:
        await ctx.send("⚙️ No hay configuración guardada.")
        return

    rD_config=config_data["rD_config"]

    embed=discord.Embed(title="⚙️ Configuración Actual",color=discord.Color.blue())
    embed.add_field(name="Mensaje",value=rD_config.get("mensaje","N/A"),inline=False)
    embed.add_field(name="Imagen",value=rD_config.get("imagen_url","N/A"),inline=False)
